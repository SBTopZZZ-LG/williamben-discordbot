...
client.once('ready', async () => {
	...
	client.on('interactionCreate', async (ic) => {
		if (ic.customId === "vote_component") {
			...
			if (Date.now() - session.createdTimestamp >= session.timeoutTimeoutDuration)
				return;

			if (ic.user.id in session.globalVoters) {
				return ic.reply({ content: "You have already casted your vote!", ephemeral: true, });
			}
			...
			if (session.timeoutMode.mode === 'c' && session.globalVotersCount >= session.timeoutMode.count) {
				updateSkip = true;
				session.timeoutCallback();
			} else {
				if (session.timeoutMode.mode === 'e') {
					let count = ic.message.guild.members.cache.filter((m) => !m.user.bot).size;
				...
				ic.message.edit({
					embeds: [
						new EmbedBuilder()
							...
							.setDescription(`\`\`\`${Object.keys(session.options).map(emoji => `${emoji} ▶ ${session.options[emoji].option}`).join("\n")}\`\`\`\n`)
							.setFooter(session.globalVotersCount === 0 ? {} : {
								text: `${(session.globalVotersCount > 6 ?
									Object.keys(session.globalVoters).slice(0, 6) : Object.keys(session.globalVoters)
								).map(globalVoter => session.globalVoters[globalVoter].name)
									.join(", ")}${session.globalVotersCount > 6 ? ` and ${session.globalVotersCount - 6} other people` : ""} have casted their vote(s)!`,
							}),
					],
				});
			}

			ic.message.reply(`<@${ic.user.id}> has casted a vote.`).then(async msg => {
				await sleep(5000);

				try {
					if (msg.deletable && await msg.channel.messages.fetch(msg.id)) msg.delete();
				} catch (e) { }
			});
		}
	...
	client.on('messageCreate', async (mc) => {
		...
		if (wbb_help.test(mc.content)) {
				...
				mc.channel.send({
					embeds: [
						new EmbedBuilder()
							...
							.setDescription("Here you can find all the wbb! commands to make me do many things for you!")
							.addFields(
								{ name: 'toh <count>', value: '```Compute the Tower of Hanoi for <count> number of discs.\nLimitations: 1 < count < 10\n\nExample: wbb!toh 3\n```' },
								{ name: "nq <size>", value: "```Compute the N-Queens for a board of size <size>.\nLimitations: 1 <= size < 10\n\nExample: wbb!nq 4```" },
								{
			...
			// Poll
			try {
				let { timeoutMode, title, options } = evalRegex(wbb_poll, mc.content);
				...
				if (_wbb_poll_timeoutMode_e.test(timeoutMode))
					...
					const { count } = evalRegex(_wbb_poll_timeoutMode_c, timeoutMode);
					const numCount = parseInt(count, 10);
					if (numCount <= 1)
						return mc.reply("Max. count cannot be any lesser than two users! 😡");
				...
				const optionsList = options.match(_wbb_poll_options).map(option => option.slice(1, option.length - 1).split("\\\"").join("\""));

				if (optionsList.length === 0)
					return mc.reply("There cannot be zero options! 😡");

				const msg = await mc.channel.send(`Quick poll incoming!`);
				const session = poll.createSession(title, optionsList, timeoutMode2, () => {
					msg.edit({
						embeds: [
							new EmbedBuilder()
								.setColor("0xefaa42")
								.setTitle(session.title)
								.setDescription(`\`\`\`${Object.keys(session.options).map(emoji => `(${session.globalVotersCount === 0 ? 0 : Math.round(session.options[emoji].votesCasted.length / session.globalVotersCount * 100)} %) ${emoji} ▶ ${session.options[emoji].option}`).join("\n")}\`\`\`\n`)
								.setFooter(session.globalVoters.length === 0 ? {} : {
									text: `This poll lasted for about ${(Date.now() - session.createdTimestamp) < 60000 ?
										`${Math.round((Date.now() - session.createdTimestamp) / 1000)} second(s)!` : (
											(Date.now() - session.createdTimestamp) < 3600000 ?
												`${Math.round((Date.now() - session.createdTimestamp) / 60000)} minute(s)!` :
												`${Math.fround((Date.now() - session.createdTimestamp) / 3600000)} hour(s)!`)}`,
								}),
						],
						components: [],
					});
				...
				if (!session || !poll.saveSession(msg.id, session)) {
					msg.delete();
					return mc.reply("Sorry, but an internal error occurred! :(");
				}
										...
										...Object.keys(session.options)
											.map(emoji =>
												new SelectMenuOptionBuilder()
													.setDefault(false)
													.setLabel(`${emoji} ▶ ${session.options[emoji].option}`)
													.setValue(emoji),
											),
									),
							),
			...
			try {
				const msg = await mc.channel.messages.fetch(mc.reference.messageId);
				...
				if (msg.deletable)
					...
					mc.reply("Message cannot be deleted! 🙁");
			} catch (e) {
				console.error(e);
			}
		}
